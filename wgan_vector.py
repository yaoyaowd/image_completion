...
from ops import batch_norm, lrelu, conv2d, conv2d_transpose, linear
...
class WGANVector(object):
    def __init__(self,
        ...
        self.g_summary = tf.summary.merge([
            self.G_summary, self.masked_images_summary,
    ...
    def generator(self, images):
        with tf.variable_scope("generator"):
            g_h0 = tf.nn.relu(self.g_bns[0](conv2d(images, 16, name='g_encode_0')))
            g_h1 = tf.nn.relu(self.g_bns[1](conv2d(g_h0, 32, name='g_encode_1')))
            g_h2 = tf.nn.relu(self.g_bns[2](conv2d(g_h1, 64, name='g_encode_2')))
            ...
            g_h4 = tf.nn.relu(self.g_bns[3](conv2d_transpose(g_h3, [self.batch_size, 8, 8, 256], name='g_h1')))
            g_h5 = tf.nn.relu(self.g_bns[4](conv2d_transpose(g_h4, [self.batch_size, 16, 16, 128], name='g_h2')))
            g_h6 = tf.nn.relu(self.g_bns[5](conv2d_transpose(g_h5, [self.batch_size, 32, 32, 64], name='g_h3')))
